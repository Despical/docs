...
public void exampleCommandMethod(CommandArguments arguments) {
    ...
    // Strings are formatted using the active component formatter.
    // MiniMessage is used by default.
    arguments.sendMessage("<red>Hey there!");

    // Sends a parameterized message directly to the sender
    arguments.sendMessage("<green>Hey there, {0}!", arguments.getSender().getName());
...
public void exampleCommandMethod(CommandArguments arguments) {
    ...
    // Send a pre-defined framework error message
    boolean success = arguments.sendMessage(CommandErrorMessage.SHORT_ARG_SIZE);
}
```
