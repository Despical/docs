---
title: Getting Started
description: Install the Easter Eggs Minecraft plugin on Paper or Spigot. Generate config files, learn the command structure, and start hiding eggs on your server in minutes.
---

import { Aside, Steps, Badge } from '@astrojs/starlight/components';

Welcome to **Easter Eggs**! This plugin allows you to hide interactive, collectible eggs around your world to keep your players entertained, featuring customizable rewards, cross-server syncing, and easy setup.

---

## Prerequisites

Before installing the plugin, ensure your server environment meets the following requirements:

* **Java Version:** [Java 21](https://www.oracle.com/java/technologies/downloads/) or newer is required.
* **Server Software:** Spigot, [**Paper**](https://papermc.io/), or any compatible fork (e.g., Purpur).
* **Minecraft Version:** 1.8 - 26.2 (Latest versions recommended).
* **Permissions Plugin** <Badge text="Optional" variant="tip" /> : A plugin like [LuckPerms](https://www.spigotmc.org/resources/luckperms.28140/) to manage user access.

---

## Installation

Setting up **Easter Eggs** is straightforward. Follow these simple steps to get started:

<Steps>
1.  **Download the Plugin:** Get the latest version of **Easter Eggs** from [SpigotMC](https://www.spigotmc.org/resources/eastereggs.116086/) or [BuiltByBit](https://builtbybit.com/resources/easter-eggs.50201/).
2.  **Upload to Server:** Place the `easter-eggs-x.x.x.jar` file into your server's `plugins/` directory.
3.  **Restart Server:** Restart your server to generate the default configuration files.
4.  **Verify Installation:** Type `/eastereggs` in-game to ensure the plugin is loaded correctly.
</Steps>

---

## First-Run Configuration

After the first restart, a new folder will be created at `/plugins/EasterEggs/`. This folder contains everything you need to customize your plugin:

* `config.yml`: Main plugin settings, storage type selection, and general behaviors.
* `messages.yml`: Full control over every string, title, and translation shown to players.
* `mysql.yml`: Configure your database credentials if you are using MySQL for cross-server synchronization.

<Aside type="tip">
    Always check your console during the first boot. If there are any missing dependencies or version conflicts, the plugin will alert you there.
</Aside>

---

## Basic Command Structure

While you'll spend most of your time in the config files, here are the essential commands:

| Command                          | Permission             | Description                                                                   |
|:---------------------------------|:-----------------------|:------------------------------------------------------------------------------|
| `/eastereggs <id>`               | `eastereggs.get`       | Gives you the specified Easter Egg item so you can place it in the world.     |
| `/eastereggs removeall <egg id>` | `eastereggs.removeall` | Deletes every placed Easter Egg matching the specified ID from the server.    |
| `/eastereggs list`               | `eastereggs.list`      | Shows a complete list of all currently configured **Easter Eggs** and their IDs.  |
| `/eastereggs reload`             | `eastereggs.reload`    | Reloads all configurations, messages, and Easter Egg data without restarting. |
| `/eastereggs help`               | `eastereggs.help`      | Displays a comprehensive list of all available commands and their usage.      |

---

## Next Steps

Now that the plugin is installed and running, it's time to hide some eggs:

* **[Creating New Eggs](/easter-eggs/creating-new-eggs)**: Learn how to define, place, and configure your first custom Easter Egg.
* **[Placeholders](/easter-eggs/placeholders/general)**: Hook into PlaceholderAPI to display player progress and total eggs found.
* **[Database Setup](/easter-eggs/configuration/database)**: Sync player data across your entire network.
