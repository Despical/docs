...
During a game, the action bar periodically reminds the player that pressing Shift leaves the arena. Dismounting the fixed camera vehicle safely follows the same leave flow.

By default, other commands are blocked while playing and `/snake leave` remains allowed. Administrators can change this policy in `config.yml`.
