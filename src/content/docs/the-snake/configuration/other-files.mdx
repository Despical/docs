---
title: Other Files
description: Understand The Snake flat-file statistics and inventory recovery behavior.
---
...
`data/stats.yml` is used when `database: flat` is selected or when the database selector is absent or invalid.
...
Do not edit `data/stats.yml` while the server is running. A later save can overwrite manual changes.
