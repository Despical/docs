...
* **Supported Versions:** Full and native support for **1.20.6 - 26.2 and all newer versions**.
...
**If the issue is caused by Multiverse-Core:**
You need to prevent it from enforcing its own default gamemode. Open your Multiverse-Core `config.yml` file, find the `enforce-gamemode` setting, and set it to `false`:
...
**If you are using a different plugin:**
Check your other plugins for similar gamemode-enforcing features and disable them. If you cannot disable them globally, make sure to bypass or disable those features specifically for the worlds where your **Santa Says** arenas are located.
