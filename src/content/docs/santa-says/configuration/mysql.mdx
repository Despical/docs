...
description: Learn how to connect the Santa Says plugin to a MySQL database for cross-server sync and data persistence.
---

import { Aside } from '@astrojs/starlight/components';

This guide explains how to configure the `mysql.yml` file to connect **Santa Says** with a MySQL database.
Below you will find detailed descriptions of every setting, ensuring a secure connection and enabling features like
cross-server synchronization.

## Configuration

By default, Santa Says stores player data using local files, but it can be configured to use a MySQL database instead.
MySQL is highly recommended for production servers as it provides better performance and reliability, while also enabling
Cross-Server Synchronization, allowing player progress to be shared across multiple servers.
...
| `<database_name>` | **Database Name:** The specific name of your database. You **must** replace this with your actual database name (e.g., `santa_says_db`). |
