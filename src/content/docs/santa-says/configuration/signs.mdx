---
title: signs.yml
description: Configure Santa Says arena signs, state labels, click behavior messages, and sign display lines.
---

The `signs.yml` file controls arena join signs. Signs can show the arena state, player limits, and any branding text you want players to see before joining.
...
```yaml
lines:
  - "<gradient:#FF1744:#D50000><bold>SANTA SAYS</gradient>"
  - "%state%"
  - ""
...
```yaml
- "<gradient:#00E676:#29B6F6><bold>%players% / %max_players%</bold></gradient>"
```
...
Breaking registered Santa Says signs requires:

```text
santasays.sign.break
```
...
3. Look at the target sign block.
4. Click the setup item to register it for the arena.

The plugin will then update the sign as the arena state changes. Run `/santasays reload` after editing `signs.yml` to apply the new layout and messages without restarting the server.
