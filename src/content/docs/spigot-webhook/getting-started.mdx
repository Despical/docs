...
Before scraping SpigotMC, send a test Discord notification:

```bash
java -jar build/libs/spigot-sales-webhook-1.0.2.jar --test-webhook
```

This confirms that `discord.webhook-url`, `discord.username`, and `discord.avatar-url` are accepted by Discord. It sends an example sale and exits.
