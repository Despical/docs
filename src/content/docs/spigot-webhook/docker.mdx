...
services:
  spigot-sales-webhook:
    build: .
    image: spigot-sales-webhook:local
...
You can pass worker arguments through Docker Compose:

```bash
docker compose run --rm spigot-sales-webhook --test-webhook
```

The container entrypoint creates the data directory, switches to the `spigotsales` user, sends the example Discord notification, and exits.
